...
function Header({ isLogged, isSignUp, onLogOut, email }) {
    return (
        <header className="header">
            ...
            <div className='header__action-wrapper'>
                {isLogged ? <div className='header__action-logged'>
                    ...
                    {isSignUp
                        ? <Link to="/" className="header__action-text">Войти</Link>
                        : <Link to="/sign-up" className="header__action-text">Регистрация</Link>}
                </div>}
            </div>
        </header>
    )
